...
struct Dinic {
    ...
    void dfs2(int x) {
        ...
        for (auto e : adj[x]) {
            if (e->remaining_cap() > 0 && !vis[e->to]) {
                dfs2(e->to);
            }
        }
    }
 
    vector<pair<int, int>> get_paths(int s, int t) {
        ...
        for (int i = 1; i <= n; i++) {
            for (auto e : adj[i]) {
                if (vis[e->from] && !vis[e->to] && !e->is_residual()) {
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ll u, v; cin >> u >> v; // não direcionado
        dinic.add_edge(u, v, 1);
        dinic.add_edge(v, u, 1);
    }
    
    ll paths = dinic.solve(1, n);
    auto ans = dinic.get_paths(1, n);
