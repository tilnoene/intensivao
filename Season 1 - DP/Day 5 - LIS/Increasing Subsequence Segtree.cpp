...
int main() {
    ...
    Segtree st(n); // seg de maximo
    
    for (int i = 0; i < n; i++) {
        int valor_comprimido = lower_bound(compressao.begin(), compressao.end(), arr[i]) - compressao.begin();
        
        int mx = st.query(0, valor_comprimido - 1);
        st.update(valor_comprimido, mx + 1);
    }
